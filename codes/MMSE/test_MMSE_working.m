...
%          %for k=1:len_frame  
             DataSym = randi([0 1],(len_frame1/2),1);
             DataSym1=reshape(DataSym,3,[]);
             DataSym2=transpose(DataSym1);
             DataSym3=zeros((len_frame/2),1);

             for ii=1:(len_frame/2)
                 DataSym3(ii)=bi2de(DataSym2(ii,:),'left-msb')+1;
             end
...
#grid on;
save ("-ascii", "mmse_ser.dat","EbN0dB","SER_MMSE","theoreticalSER","theory_bpsk");
